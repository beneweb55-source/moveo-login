...
const Films = () => {
  ...
  // Pagination failure handling — see fetchNextPageData. Two guards, because a
  // rejected Promise.all used to leave the loader spinning AND leave pageRef
  // unadvanced, so InfiniteScroll re-fired `next` against the same failing page.
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      ...
      const res = await fetchDataFromApi(`/genre/movie/list`, { language: langParam });
      if (res?.genres) {
  ...
  useEffect(() => {
    const fetchInitialData = () => {
      ...
      Promise.all([
        ...
        fetchDataFromApi(`/discover/movie`, { ...params, page: 3 })
        ...
        console.error("[films] failed to load the initial catalogue:", error);
      }).finally(() => {
        setLoading(false);
      });
    };
  ...
  const fetchNextPageData = () => {
    ...
    Promise.all([
      fetchDataFromApi(`/discover/movie`, { ...params, page: pageRef.current }),
      fetchDataFromApi(`/discover/movie`, { ...params, page: pageRef.current + 1 })
    ]).then(
      ([page1, page2]) => {
      ...
      console.error("[films] failed to load more of the catalogue:", error);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      <HeroBanner endpoint="/discover/movie" params={{ "vote_count.gte": 50 }} />
